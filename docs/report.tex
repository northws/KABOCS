...
\begin{itemize}
    ...
    \item \textbf{任务参数}：\texttt{task}（记录 \texttt{task\_name()}）、\texttt{selected\_features}（实际使用的 $K$ 个描述符）
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{异构特征类型}：当前 GP 仅处理连续特征，离散类别特征（如"溶剂种类"）可通过 Wang et al. CatBOX 框架\cite{catbox2025} 的谱混合核扩展
