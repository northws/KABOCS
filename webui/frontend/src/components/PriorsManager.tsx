...
export default function PriorsManager() {
  ...
  async function remove(path: string) {
    if (!confirm(t("priors.delete.confirm", { path }))) return;
    try {
    ...
    <div className="grid grid-cols-1 xl:grid-cols-[320px_1fr] gap-6">
      <aside className="rounded-lg border border-slate-200 bg-white">
        ...
        <ul className="max-h-[420px] overflow-auto log-scroll">
          {files.length === 0 ? (
            <li className="p-3 text-xs text-slate-400">{t("priors.none")}</li>
          ) : (
            files.map((f) => {
        ...
        <div className="p-3 border-t">
          <div className="flex gap-2">
            ...
            >
              <FilePlus2 className="w-3.5 h-3.5" /> {t("priors.new")}
            </button>
          </div>
        </div>
      </aside>
      ...
      <section className="rounded-lg border border-slate-200 bg-white flex flex-col min-w-0">
        <header className="px-4 py-3 border-b bg-slate-50 flex items-center gap-3">
          ...
          >
            ...
            {saving ? t("priors.saving") : t("priors.save")}
          </button>
        </header>
        <textarea
          value={text}
          ...
          placeholder={t("priors.placeholder")}
          disabled={!selected}
        />
        {msg ? (
